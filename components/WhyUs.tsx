      ...
      <div className="mx-auto mt-12 grid max-w-5xl gap-5 sm:grid-cols-3">
        {PILLARS.map((pillar, i) => (
          <FadeIn key={pillar.number} variant={CARD_VARIANTS[i]} delay={i * 120}>
            <div className="h-full rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
              <span className="font-heading text-3xl font-black text-brand-blue">{pillar.number}</span>
              <h3 className="mt-3 font-heading text-base font-bold leading-snug text-neutral-900">
                {pillar.title}
              </h3>
